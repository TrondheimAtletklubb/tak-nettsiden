...
test.describe('Visual Regression Tests', () => {
  ...
  test('pricing section should look correct', async ({ page }) => {
    ...
    await page.evaluate((element) => element.scrollIntoView(), await pricing.elementHandle());
  ...
  test('mobile navigation should work', async ({ page }) => {
    ...
    // Test closed state
    await expect(page.locator('[class*="navigation"]').first()).toHaveScreenshot('mobile-nav-closed.png');
    
    // Test open state
    await burgerButton.click();
    await expect(page.locator('[class*="navigation"]').first()).toHaveScreenshot('mobile-nav-open.png');
  });
